...
# GPT 모델
llm = ChatOpenAI(model="gpt-4o", temperature=0.3, api_key=api_key)  # temperature를 높여 더 길게 생성
...
"""
    ...
    if inputs.word_count:
        # 단순하고 직접적인 지시
        purpose_word_count = f"\n\n━━━━━━━━━━━━━━━━━━━━━━\n최우선 목표: 본문 최소 {inputs.word_count}자 이상\n━━━━━━━━━━━━━━━━━━━━━━"
        
        length_instructions = f"""
━━━━━━━━━━━━━━━━━━━━━━━━━━
[본문 길이 규칙 - 반드시 준수]
━━━━━━━━━━━━━━━━━━━━━━━━━━
1. 본문 전체: 최소 {inputs.word_count}자 이상 (공백 포함)
2. 문단 수: 최소 10개 문단 작성
3. 각 문단 길이: 평균 400-600자 (짧으면 안됨)
4. 절대 간결하게 쓰지 않기
5. 모든 내용을 최대한 상세하고 풍부하게 확장하세요.
6. 예시, 수치, 구체적 상황을 많이 포함하세요.
7. 한 문장을 두세 문장으로 확장하여 쓰세요.
━━━━━━━━━━━━━━━━━━━━━━━━━━"""
...
1) 제목: 추천서
2) 빈 줄
3) 본문 (8-10개 이상의 긴 문단, 각 문단 300-500자)
   - 작성자 소개와 관계
   - 첫 인상과 전반적 역량 평가
   - 구체적 성과 사례 1 (상세히)
   - 구체적 성과 사례 2 (상세히)
...
[추천 강도]
- 점수 {score}에 맞게 마지막 문단의 추천 어조를 조절하세요.
- 1-2점: 신중한 추천, 3점: 명확한 추천, 4-5점: 강력한 추천
...
async def generate(request: RecommendationRequest):
    ...
    try:
        ...
        # OpenAI API 에러 객체에서 상세 정보 추출 시도
        error_detail = None
        error_code = None
        error_type_name = None
        
        try:
            # OpenAI API 에러 객체인 경우
            if hasattr(e, 'response'):
                if hasattr(e.response, 'json'):
                    error_detail = e.response.json()
                    print(f"OpenAI API 응답: {error_detail}")
            ...
            # LangChain이 래핑한 경우 또는 OpenAI 라이브러리 직접 사용
            elif hasattr(e, 'status_code'):
                print(f"HTTP 상태 코드: {e.status_code}")
            # OpenAI 에러 객체의 다른 속성 확인
            if hasattr(e, 'body'):
                try:
        ...
        # OpenAI는 RateLimitError로 래핑하지만 실제로는 quota 문제일 수 있음
        is_quota = (
        ...
        if is_quota:
            raise HTTPException(
                status_code=503,
                detail="OpenAI API 사용량 한도를 초과했습니다. OpenAI 계정의 플랜 및 결제 정보를 확인해주세요. (Error: Insufficient Quota)"
            )
        elif is_rate_limit:
            raise HTTPException(
                status_code=429,
        ...
        # 429 에러이지만 rate_limit도 quota도 아닌 경우
            ...
            if "quota" in error_msg.lower():
                raise HTTPException(
                    status_code=503,
                    detail="OpenAI API 사용량 한도를 초과했습니다. OpenAI 계정의 플랜 및 결제 정보를 확인해주세요."
                )
            else:
                raise HTTPException(
                    status_code=429,
        ...
        # 기타 OpenAI API 에러 처리
        elif "openai" in error_msg.lower() or "api" in error_msg.lower() or error_type.startswith("OpenAI") or "RateLimitError" in error_type:
            raise HTTPException(
                status_code=503,
