...
# ▼ 기존 evals 시스템 import - 동적 import로 변경 (IDE 경고 방지)
...
if not openai_api_key:
    ...
    print(f"✅ OPENAI_API_KEY 로드됨: {openai_api_key[:20]}...{openai_api_key[-4:]}")
...
# 문서 파일 임시 저장 디렉토리
DOCUMENTS_DIR = os.path.join(STATIC_DIR, "documents", "samples")
...
def analyze_writing_style_with_ai(text: str) -> dict:
    """
    AI를 사용하여 텍스트의 문체 분석
    Claude API 사용 - 향상된 분석 (speech_level, subject_style, sentence_flow 추가)
    """
...
━━━━━━━━━━━━━━━━━━━━━━━━━━"""
    ...
    # 문체 반영이 있으면 프롬프트 시작 부분에 강력하게 추가 (AI 자동 판단 방식)
    style_prefix = ""
    if writing_style:
...
- 추천서 톤: {inputs.tone}
  * 톤 종류 및 특징:
    - 공식적 (Formal):
      • 끝맺음: ~입니다, ~습니다, ~하였습니다, ~것입니다
      • 연결어: 따라서, 그러므로, 또한, 더불어, 이에, 한편
      • 특징: 존댓말 철저, 전문 용어 사용, 객관적 서술, 격식있는 표현
      
    - 친근한 (Friendly):
      • 끝맺음: ~해요, ~이에요, ~했어요, ~거예요
      • 연결어: 그래서, 그리고, 또, 정말, 특히, 무엇보다
      • 특징: 따뜻한 어조, 일상적 표현, 개인적 감정 포함, 친밀감 표현
      
    - 간결한 (Concise):
      • 끝맺음: ~함, ~임, ~했음, ~것임 (명사형 종결)
      • 연결어: 또한, 그리고, 특히 (최소한만 사용)
      • 특징: 짧은 문장, 핵심만 전달, 수식어 제거, 직설적 표현
      
    - 설득형 (Persuasive):
      • 끝맺음: ~합니다, ~할 것입니다, ~확신합니다, ~추천합니다
      • 연결어: 실제로, 특히, 뿐만 아니라, 더욱이, 나아가, 결과적으로
      • 특징: 강한 긍정 표현, 구체적 증거 제시, 확신 어조, 능동적 추천
...
def generate_single_score_recommendation(inputs: RecommendationRequest, score: int, recommender_email: str = "", user_details: dict = None, template_content: str = None, writing_style: dict = None, max_retries: int = 3) -> str:
    ...
    if writing_style:
        print("=" * 50)
        print("🎨 문체 반영된 프롬프트 생성됨!")
        print(f"끝맺음 표현: {writing_style.get('common_phrases', [])}")
        print("프롬프트 시작 부분:")
        print(prompt[:500])
        print("=" * 50)
...
async def upload_writing_sample(
    ...
    # 불필요한 필드 제거 (문장 길이, 어휘 수준, 문단 스타일)
    filtered_style = {
        "tone": style_analysis.get("tone"),
        "common_phrases": style_analysis.get("common_phrases", []),
        "characteristics": style_analysis.get("characteristics", [])
    }
    
    # 추가 필드도 유지 (있는 경우)
    if "sentence_endings" in style_analysis:
        filtered_style["sentence_endings"] = style_analysis.get("sentence_endings", [])
    if "connectors" in style_analysis:
        filtered_style["connectors"] = style_analysis.get("connectors", [])
    ...
    try:
        with engine.connect() as conn:
            ...
            style_json = json.dumps(filtered_style, ensure_ascii=False)
...
async def get_my_writing_style(current_user: dict = Depends(get_current_user)):
    ...
    try:
        with engine.connect() as conn:
            ...
            style_analysis = json.loads(row[0]) if row[0] else {}
...
async def transcribe_audio(audio_file: UploadFile) -> str:
    ...
    try:
        ...
        if audio_file.filename:
            ...
            if "m4a" in audio_file.content_type or "mp4" in audio_file.content_type:
                file_ext = "m4a"
            elif "webm" in audio_file.content_type:
                file_ext = "webm"
            elif "mpeg" in audio_file.content_type or "mp3" in audio_file.content_type:
                file_ext = "mp3"
            elif "wav" in audio_file.content_type:
                file_ext = "wav"
        
        print(f"🎤 음성 파일 형식: {file_ext}")
        
        # 임시 파일로 저장 (동적 확장자)
        temp_file_path = os.path.join(AUDIO_TEMP_DIR, f"temp_{datetime.now().timestamp()}.{file_ext}")
        
        with open(temp_file_path, "wb") as f:
            content = await audio_file.read()
            f.write(content)
        
        print(f"📁 임시 파일 저장: {temp_file_path} ({len(content)} bytes)")
        
        # Whisper API로 변환 (모든 형식 자동 지원)
        ...
        # Whisper가 텍스트로 반환하면 그대로, 객체면 .text 속성 사용
...
async def generate(request: RecommendationRequest):
    ...
    writing_style = None
    try:
        print(f"🔍 문체 조회 시작 - 작성자 userId: {from_user.id}")
        with engine.connect() as conn:
            # deletedAt 조건 제거 (테이블에 없을 수 있음)
            style_sql = sql_text("""
                SELECT styleAnalysis
                FROM writing_styles
                WHERE userId = :user_id
                LIMIT 1
            """)
            style_row = conn.execute(style_sql, {"user_id": from_user.id}).first()
            print(f"🔍 쿼리 실행 결과: {style_row}")
            
            if style_row and style_row[0]:
                writing_style = json.loads(style_row[0])
                print(f"✅ 작성자 문체 로드 완료: {writing_style.get('tone', 'N/A')}")
                print(f"   끝맺음: {', '.join(writing_style.get('common_phrases', []))}")
            else:
                print(f"⚠️ 작성자의 문체 정보 없음 (userId={from_user.id}) - 기본 스타일로 생성")
                # 전체 데이터 확인
                all_styles = conn.execute(sql_text("SELECT userId, originalFilename FROM writing_styles")).fetchall()
                print(f"   DB에 있는 전체 문체 데이터: {all_styles}")
    except Exception as e:
        print(f"문체 조회 오류 (계속 진행): {e}")
        import traceback
        traceback.print_exc()
...
async def login(user: UserLogin):
    with engine.connect() as conn:
        result = conn.execute(
            sql_text("SELECT * FROM users WHERE email = :email AND deletedAt IS NULL"),
            {"email": user.email},
        ).first()
        if not result:
...
async def get_user_details(user_id: int, requester_email: Optional[str] = None):
    ...
    requester_email: 요청자의 이메일 (권한 확인용, 필수)
    """
    try:
        with engine.connect() as conn:
            # requester_email이 없으면 권한 확인 불가
            if not requester_email:
                raise HTTPException(
                    status_code=401, 
                    detail="요청자 이메일이 필요합니다."
                )
            
            # 1) 본인 확인
            user_sql = sql_text("SELECT email FROM users WHERE id = :user_id AND deletedAt IS NULL")
            user = conn.execute(user_sql, {"user_id": user_id}).first()
            
            if not user:
                raise HTTPException(status_code=404, detail="사용자를 찾을 수 없습니다.")
            
            is_owner = user._mapping.get("email") == requester_email
            
            print(f"=== 권한 체크 디버깅 ===")
            print(f"user_id: {user_id}")
            print(f"owner_email: {user._mapping.get('email')}")
            print(f"requester_email: {requester_email}")
            print(f"is_owner: {is_owner}")
            
            if not is_owner:
                # 2) 권한 확인 (본인이 아닌 경우)
                permission_sql = sql_text("""
                    SELECT id, userId, ownerEmail, allowedEmail, deletedAt 
                    FROM userDetailPermissions
                    WHERE (userId = :user_id OR ownerEmail = :owner_email)
                    AND allowedEmail = :requester_email
                """)
                permission = conn.execute(permission_sql, {
                    "user_id": user_id,
                    "owner_email": user._mapping.get("email"),
                    "requester_email": requester_email
                }).first()
                
                print(f"권한 쿼리 파라미터: user_id={user_id}, owner_email={user._mapping.get('email')}, requester_email={requester_email}")
                print(f"권한 조회 결과: {permission}")
                if permission:
                    print(f"  - id: {permission._mapping.get('id')}")
                    print(f"  - userId: {permission._mapping.get('userId')}")
                    print(f"  - ownerEmail: {permission._mapping.get('ownerEmail')}")
                    print(f"  - allowedEmail: {permission._mapping.get('allowedEmail')}")
                    print(f"  - deletedAt: {permission._mapping.get('deletedAt')}")
                
                # deletedAt이 NULL인 경우만 유효한 권한
                if not permission or permission._mapping.get('deletedAt') is not None:
                    print(f"권한 없음 또는 삭제됨")
                    print(f"===================")
                    raise HTTPException(
                        status_code=403, 
        ...
        # HTTPException은 그대로 re-raise (FastAPI가 처리)
        raise
    except Exception as e:
...
class GrantPermissionRequest(BaseModel):
    user_id: Optional[int] = None       # 상세정보 소유자 ID (선택)
    user_email: Optional[EmailStr] = None  # 상세정보 소유자 이메일 (선택)
    allowed_email: EmailStr             # 허용할 이메일
    note: Optional[str] = None          # 메모 (예: "이교수님 추천서용")

class RevokePermissionRequest(BaseModel):
    user_id: Optional[int] = None       # 상세정보 소유자 ID (선택)
    user_email: Optional[EmailStr] = None  # 상세정보 소유자 이메일 (선택)
    allowed_email: EmailStr             # 취소할 이메일

@app.post("/grant-detail-permission")
async def grant_detail_permission(request: GrantPermissionRequest):
    """
    특정 이메일에게 내 상세정보 조회 권한을 부여합니다. (user_id 또는 user_email 중 하나 필수)
    회원가입 안 한 이메일도 등록 가능 - 나중에 가입하면 자동 연결됨
    """
    try:
        with engine.connect() as conn:
            # user_email 우선 사용
            ...
            if request.user_email:
                ...
                # 회원가입 여부 확인 (있으면 userId도 설정)
                user_sql = sql_text("SELECT id FROM users WHERE email = :email AND deletedAt IS NULL")
                user = conn.execute(user_sql, {"email": request.user_email}).first()
                if user:
                    user_id = user._mapping.get("id")
            elif request.user_id:
                # user_id만 있는 경우 (하위 호환성)
                ...
                raise HTTPException(status_code=400, detail="user_id 또는 user_email 중 하나를 제공해야 합니다.")
            
            if not owner_email:
                raise HTTPException(status_code=400, detail="소유자 이메일을 확인할 수 없습니다.")
            
            # 권한 부여 (ownerEmail 기반, userId는 있으면 설정)
            # 기존에 삭제된 레코드가 있으면 재활성화
            sql = sql_text("""
                INSERT INTO userDetailPermissions (userId, ownerEmail, allowedEmail, note, createdAt, updatedAt, deletedAt)
                VALUES (:user_id, :owner_email, :allowed_email, :note, NOW(), NOW(), NULL)
            ...
            conn.execute(sql, {
                ...
                "allowed_email": request.allowed_email,
                "note": request.note or ""
            })
            conn.commit()
            
        return {
            "message": f"{request.allowed_email}에게 상세정보 조회 권한을 부여했습니다.",
            "success": True
        }
    except Exception as e:
        import traceback
        ...
        print(f"스택 트레이스:")
        traceback.print_exc()
        print(f"요청 데이터: user_email={request.user_email}, user_id={request.user_id}, allowed_email={request.allowed_email}")
        print(f"=====================")
        raise HTTPException(status_code=500, detail=f"권한 부여에 실패했습니다: {str(e)}")

@app.get("/my-permissions/{user_id}")
async def get_my_permissions(user_id: int = None, user_email: str = None):
    """내가 부여한 상세정보 조회 권한 목록을 조회합니다. (user_id 또는 user_email)"""
    try:
        with engine.connect() as conn:
            # user_email 우선 사용
            if user_email:
                ...
                """)
                rows = conn.execute(sql, {"owner_email": user_email}).fetchall()
            elif user_id:
                sql = sql_text("""
                    SELECT allowedEmail, note, createdAt
                    FROM userDetailPermissions
                    WHERE (userId = :user_id OR ownerEmail = (SELECT email FROM users WHERE id = :user_id)) 
            ...
            for row in rows:
                permissions.append({
                    "allowedEmail": row._mapping.get("allowedEmail"),
                    "note": row._mapping.get("note"),
                    "createdAt": row._mapping.get("createdAt").strftime('%Y-%m-%d %H:%M:%S') if row._mapping.get("createdAt") else None
                })
        ...
        raise HTTPException(status_code=500, detail="권한 목록 조회에 실패했습니다.")

@app.post("/revoke-detail-permission")
async def revoke_detail_permission(request: RevokePermissionRequest):
    """특정 이메일에게 부여한 상세정보 조회 권한을 취소합니다. (완전 삭제)"""
    try:
        with engine.connect() as conn:
            # user_email 우선 사용
            # UNIQUE KEY 제약 때문에 소프트 삭제 대신 완전 삭제
            if request.user_email:
                sql = sql_text("""
                    DELETE FROM userDetailPermissions
                    WHERE ownerEmail = :owner_email AND allowedEmail = :allowed_email AND deletedAt IS NULL
                """)
                ...
                sql = sql_text("""
                    DELETE FROM userDetailPermissions
                    WHERE (userId = :user_id OR ownerEmail = (SELECT email FROM users WHERE id = :user_id))
                    AND allowedEmail = :allowed_email AND deletedAt IS NULL
                """)
                result = conn.execute(sql, {
                    "user_id": request.user_id,
                    "allowed_email": request.allowed_email
            ...
            return {
                "message": f"{request.allowed_email}의 조회 권한을 취소했습니다.",
                "success": True
            }
    except HTTPException:
        raise
        ...
        traceback.print_exc()
        print(f"=====================")
        raise HTTPException(status_code=500, detail=f"권한 취소에 실패했습니다: {str(e)}")

@app.get("/check-detail-permission/{user_id}")
async def check_detail_permission(user_id: int, requester_email: str):
    """
    특정 사용자의 상세정보를 볼 권한이 있는지 확인합니다.
    프론트엔드에서 "상세정보 보기 가능" 표시용
    """
    try:
        with engine.connect() as conn:
            # 1) 본인 확인
            ...
            if user and user._mapping.get("email") == requester_email:
                return {
                    "hasPermission": True,
                    "reason": "본인"
                }
            ...
            if permission:
                return {
                    "hasPermission": True,
                    "reason": "권한 부여됨",
                    "note": permission._mapping.get("note")
                }
        ...
        print(f"권한 확인 오류: {e}")
        raise HTTPException(status_code=500, detail="권한 확인에 실패했습니다.")
...
async def check_email_available(email: str = Query(..., min_length=3)):
    ...
    """
    with engine.connect() as conn:
        row = conn.execute(sql_text("SELECT id FROM users WHERE email = :email AND deletedAt IS NULL"),
                           {"email": email}).first()
...
async def signup_step1(payload: SignupStep1Request):
    ...
    with engine.connect() as conn:
        # 이메일 중복
        exists = conn.execute(sql_text("SELECT id FROM users WHERE email = :email AND deletedAt IS NULL"),
...
async def signup_step2(payload: SignupStep2Request):
    ...
    with engine.connect() as conn:
        # 사용자 존재
        u = conn.execute(sql_text("SELECT id FROM users WHERE id = :uid AND deletedAt IS NULL"),
...
async def update_profile_info(
    ...
    with engine.begin() as conn:
        ...
        if pwd is not None:
            ...
            # 이전 비밀번호와 동일 여부 검사
            cur = conn.execute(sql_text("SELECT password FROM users WHERE id = :uid AND deletedAt IS NULL LIMIT 1"), {"uid": current_user["id"]}).first()
...
async def create_reputation(payload: ReputationCreate, current_user: dict = Depends(get_current_user)):
    ...
    try:
        with engine.begin() as conn:
            # 대상 사용자 존재 확인
            target_user = conn.execute(sql_text("""
                SELECT id FROM users
        ...
        with engine.connect() as conn:
            sql = sql_text("""
                SELECT
                    ...
                    u_to.nickname AS requester_name,
                    u_to.email AS requester_email
                FROM recommendation r
                JOIN users u_to ON u_to.id = r.toUserId
                WHERE r.fromUserId = :user_id AND r.deletedAt IS NULL
                ORDER BY r.createdAt DESC
            """)
            rows = conn.execute(sql, {"user_id": user_id}).fetchall()
            ...
            for row in rows:
                recommendations.append({
                    ...
                    "created_at": row._mapping.get("created_at").strftime('%Y-%m-%d %H:%M:%S') if row._mapping.get("created_at") else "",
                    "requester_name": row._mapping.get("requester_name"),
                    "requester_email": row._mapping.get("requester_email"),
                })
            
            return {"recommendations": recommendations, "total": len(recommendations)}
    except Exception as e:
        print(f"추천서 목록 조회 오류: {e}")
        raise HTTPException(status_code=500, detail="추천서 목록 조회 실패")
...
async def my_reputations_sent(current_user: dict = Depends(get_current_user)):
    """로그인한 사용자가 작성한 평판 목록을 조회합니다."""
    with engine.connect() as conn:
        rows = conn.execute(sql_text("""
            SELECT 
                r.id, r.rating, r.comment, r.category, r.createdAt,
                u_target.nickname AS target_name, u_target.email AS target_email
            FROM userReputations r
            JOIN users u_target ON u_target.id = r.userId
            WHERE r.deletedAt IS NULL AND r.fromUserId = :uid
            ORDER BY r.createdAt DESC
        """), {"uid": current_user["id"]}).fetchall()
    items = []
...
async def get_recommendation_detail(recommendation_id: int, current_user: dict = Depends(get_current_user)):
    """특정 추천서의 상세 정보를 조회합니다."""
    try:
        with engine.connect() as conn:
            rec_sql = sql_text("""
                SELECT 
                    r.id, r.content, r.createdAt, r.signatureData,
                    u_from.id AS from_user_id, u_from.nickname AS from_name, u_from.email AS from_email,
                    u_to.id AS to_user_id, u_to.nickname AS to_name, u_to.email AS to_email
                FROM recommendation r
                JOIN users u_from ON u_from.id = r.fromUserId
                JOIN users u_to ON u_to.id = r.toUserId
                WHERE r.id = :rec_id AND r.deletedAt IS NULL
                  AND (r.fromUserId = :user_id OR r.toUserId = :user_id)
            """)
            rec = conn.execute(rec_sql, {"rec_id": recommendation_id, "user_id": current_user["id"]}).first()
            ...
            # 서명 데이터 파싱
            signature_data = None
            if rec._mapping.get("signatureData"):
                try:
                    signature_data = json.loads(rec._mapping.get("signatureData"))
                except:
                    pass
            
            return {
                "id": rec._mapping.get("id"),
                "content": rec._mapping.get("content"),
                "created_at": rec._mapping.get("createdAt").strftime('%Y-%m-%d %H:%M:%S') if rec._mapping.get("createdAt") else "",
                "from_user_id": rec._mapping.get("from_user_id"),
                "from_name": rec._mapping.get("from_name"),
                "from_email": rec._mapping.get("from_email"),
                "to_user_id": rec._mapping.get("to_user_id"),
                "to_name": rec._mapping.get("to_name"),
                "to_email": rec._mapping.get("to_email"),
                "requester_name": rec._mapping.get("to_name"),
                "requester_email": rec._mapping.get("to_email"),
...
async def download_pdf(recommendation_id: int, current_user: dict = Depends(get_current_user)):
    ...
    try:
        with engine.connect() as conn:
            ...
            # 보라색으로 제목 색상 설정
            c.setFillColorRGB(0.58, 0.44, 0.86)  # #9370DB (보라색)
            title = "추천서" if font_registered else "Recommendation Letter"
            c.drawCentredString(width / 2, height - 80, title)
            
            # 제목 아래 보라+노랑 그라데이션 느낌의 선 그리기
            y_line = height - 95
            # 보라색 선
            c.setStrokeColorRGB(0.58, 0.44, 0.86)  # #9370DB
            c.setLineWidth(2)
            c.line(100, y_line, width - 100, y_line)
            # 노란색 선
            c.setStrokeColorRGB(1.0, 0.84, 0.0)  # #FFD700
            c.setLineWidth(1)
            c.line(100, y_line - 3, width - 100, y_line - 3)
            ...
            y_position = height - 160  # 제목과 선이 추가되어 시작 위치 조정
            line_height = 18
            max_width = width - 100
...
class VerifyRequest(BaseModel):
    recommendation_text: str
    context: Optional[str] = None  # 추가 컨텍스트 (관계, 강점 등)

class VerifyResponse(BaseModel):
    is_appropriate: bool  # 적절한지 여부
    confidence: float  # 신뢰도 (0-1)
    feedback: str  # AI의 피드백
    suggestions: List[str]  # 구체적인 제안사항
...
@app.post("/verify-recommendation", response_model=VerifyResponse)
async def verify_recommendation(request: VerifyRequest):
    """
    추천서를 AI에게 한번 더 검증하고 피드백을 받습니다.
    """
    print("=== AI 재검증 요청 받음 ===")
    print(f"추천서 길이: {len(request.recommendation_text)} 자")
    ...
    try:
        if not claude_api_key:
            raise HTTPException(
                status_code=503,
                detail="AI 검증 시스템을 사용할 수 없습니다. ANTHROPIC_API_KEY가 설정되지 않았습니다."
            )
        
        # Claude에게 추천서 검증 요청
        verify_prompt = f"""다음 추천서를 전문가 관점에서 검증해주세요.

추천서:
{request.recommendation_text}

{f"추가 컨텍스트: {request.context}" if request.context else ""}

다음 항목들을 평가하고 피드백을 제공해주세요:
1. 이 추천서가 실제 추천서로 적절한가?
2. 내용이 설득력 있고 구체적인가?
3. 문법이나 표현에 문제는 없는가?
4. 개선할 수 있는 구체적인 부분은?
...
{{
  ...
  "suggestions": ["구체적인 개선 제안 1", "구체적인 개선 제안 2", ...]
        ...
        return {
            ...
            "feedback": result.get("feedback", "검증 완료"),
...
async def extract_text_from_file(file: UploadFile) -> str:
    """업로드된 파일에서 텍스트를 추출합니다 (TXT, DOCX, PDF 지원)"""
...
def analyze_writing_style_with_ai(text: str) -> dict:
    """Claude AI를 사용하여 텍스트의 문체를 분석합니다"""
    if len(text) < 100:
        raise ValueError("분석하기에 텍스트가 너무 짧습니다 (최소 100자 필요)")
    
    # 분석용 샘플 (너무 길면 처음 5000자만)
    sample = text[:5000] if len(text) > 5000 else text
    
    analysis_prompt = f"""
다음 텍스트의 문체를 분석해서 JSON 형식으로 반환해주세요.
...
{{
  "tone": "어조 (예: 격식체, 반말체, 구어체, 문어체 등)",
  "sentence_endings": ["자주 사용하는 문장 끝맺음 표현 5개", "예: ~입니다, ~해요, ~네요"],
  "connectors": ["자주 사용하는 연결어 5개", "예: 그래서, 하지만, 또한"],
  "common_phrases": ["자주 사용하는 특징적 표현 5개"],
  "characteristics": ["문체 특징을 간결하게 3-5개 (예: 직설적 표현, 비유 사용 등)"]
}}

**중요**: characteristics는 연령대나 세대 표현 없이 문체 자체의 특징만 간결하게 작성하세요.
"""
